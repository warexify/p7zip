...
" 3.13  Copyright (c) 1999-2003 Igor Pavlov  2003-12-10\n";
...
int Main2(int numArguments, const char *arguments[])
{
  ...
  try
  {
    if (kSwitchForms[0].IDString == 0) //  FIXME
    {
      printf("ERROR : Buggy compiler like gcc 3.3.2 for Mandrake 10.0 Official ?\n");
      printf("        compile with -g2 instead of -O2 ...\n");
      exit(EXIT_FAILURE);
    }
  ...
  else if(command.IsFromUpdateGroup())
  {
    ...
    if (result != S_OK) {
	  printf("Main : throw NExitCode::CSystemError(%d)\n",result);
