    ...
    const CCompressionMethodMode *options, UINT64 &headerOffset)
{
  ...
  UINT64 externalFoldersStreamIndex = 0;
  ...
  bool externalNames = (compressHeaders && database.Files.Size() > 8);
  {
    ...
    for(i = 0; i < database.Files.Size(); i++)
      namesDataSize += (database.Files[i].Name.Length() + 1) * 2; // FIXED sizeof(wchar_t) == 4 with GCC
    ...
    for(i = 0; i < database.Files.Size(); i++)
    {
      ...
      if (sizeof(wchar_t) == 2)
      {
         memmove(namesData + pos, name, length);
	      ...
	      {
		      vec.push_back((unsigned short)(name[ind]));
	      }
         length = name.Length() * 2; // FIXED
         memmove(namesData + pos, &vec[0], length);
      }
      pos += length;
      namesData[pos++] = 0;
      namesData[pos++] = 0;
  ...
  UINT64 externalLastWriteTimeStreamIndex = 0; // FIXED
