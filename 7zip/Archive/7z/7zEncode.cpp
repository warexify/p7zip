...
{
  _mixerCoderSpec = new NCoderMixer2::CCoderMixer2; // launch MainCoderThread()
  ...
  for (int i = 0; i < _options.Methods.Size(); i++)
  {
	...
	for(int ind=0;ind < _options.Password.Length() ;ind++) {
	   vec.push_back((unsigned short)(_options.Password[ind]));
	}
        RINOK(cryptoSetPassword->CryptoSetPassword(
          (const BYTE *)(&vec[0]), 
