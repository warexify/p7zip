...
CCoderMixer2::~CCoderMixer2()
{
  _exitEvent.Set();
  _mainThread.Wait();
  for(int i = 0; i < _threads.Size(); i++) _threads[i].WaitAndClose();
}
...
void CCoderMixer2::AddCoderCommon()
{
  ...
  // DWORD id;
  //HANDLE newThread = ::myCreateThread(NULL, 0, CoderThread,&_coderInfoVector.Back(), 0, &id);
  //if (newThread == 0) throw 271824;_threads.Add(newThread);
  NWindows::CThread newThread;
  _threads.Add(newThread);
  if (_threads.Back().Create(CoderThread,&_coderInfoVector.Back()) == false)
	throw 271824;
}

void CCoderMixer2::AddCoder(ICompressCoder *coder)
{
  AddCoderCommon(); // launch CoderThread
...
bool CCoderMixer2::MyCode()
{
  ...
  DWORD result = ::myWaitForMultipleEvents(_compressingCompletedEvents.Size(), 
